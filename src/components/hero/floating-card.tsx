...
export function FloatingCard({ card, style, delay }: FloatingCardProps) {
  ...
  const t = (config: Record<string, unknown>) =>
  ...
  return (
    <motion.div
      ...
      style={{
        ...style,
        background: isDark ? 'rgba(26, 31, 37, 0.95)' : 'rgba(248, 249, 250, 0.95)',
        borderColor: isDark ? 'rgba(154, 173, 158, 0.3)' : 'rgba(143, 163, 146, 0.25)',
        boxShadow: isDark ? '0 8px 32px rgba(0, 0, 0, 0.3)' : '0 8px 32px rgba(143, 163, 146, 0.1)',
      }}
      initial={{ opacity: 0, y: 30, scale: 0.9 }}
      animate={reduce ? { opacity: 1, y: 0, scale: 1 } : { y: [-8, 0, -8], opacity: 1, scale: 1 }}
      transition={t({ duration: 5, repeat: Infinity, ease: 'easeInOut', delay })}
      whileHover={{
        scale: 1.05,
        rotate: 1,
      ...
      <div className="relative p-5">
        <div className="relative inline-block mb-4">
          <motion.div
            ...
            animate={reduce ? {} : { scale: [1, 1.15, 1], opacity: [0.2, 0.4, 0.2] }}
            transition={t({ duration: 3, repeat: Infinity, ease: 'easeInOut' })}
          />
          <motion.div
            className={`relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${card.color} shadow-md`}
            whileHover={{ rotate: 10, scale: 1.08 }}
            transition={t({ type: 'spring', stiffness: 400, damping: 10 })}
          >
        ...
        <div className="space-y-2">
          ...
          <div className="h-1.5 bg-muted/60 rounded-full overflow-hidden">
            <motion.div
              ...
              transition={t({ duration: 1.5, delay: delay + 0.5, ease: 'easeOut' })}
            />
          </div>
        </div>
      </div>
